...
baranchor <- patients_tble %>%
  ...
  geom_bar(mapping = aes (fill = gender, x = anchor_age)) +
...
labevents_tble %>%
  left_join(select(icustays_tble, subject_id, intime, outime),
    by = c("subject_id")) %>%
  ...
  pivot_wider(names_froom = itemid, values_from = valuenum) %>%
