...
```{r}
admissions_tble <- admissions_tble %>%
  mutate( "lengthstay"= ((dischtime - admittime)/2400)/30
        )
admissions_tble

 
ggplot(data = admissions_tble) + 
  geom_bar(fill = "pink", mapping = aes(x = lengthstay)) + xlim(-2,10) +ylim(0,10) 
  labs(title = "Bar plot for lengthstay in hospital", 
      x = "Lengthstay(in months)") 
range(admissions_tble$lengthstay)
```
**Solution**
I plot the hospital stay data in month. When we look at the range of the graph, 
we can see that there are some negative values which could be some wrong records.
I keep them in the result.  
