...
```
```{bash}
#show files in the mimic folder
ls ~/mimic
```
...
```
**Sollution** It finds out all the datafiles in the folder "~/mimic/core/" that ends with a name of "gz" and print the names and information of them out one by one, as a long format with it "ls" information results.
...
```
**Sollution: ** : 523741 in admissions.csv.gz,   382279 in patients.csv.gz,  2189536, transfers.csv.gz. 
...
```{bash}
zless /Users/Aoki/mimic/core/admissions.csv.gz | head -10

```
**Sollution: ** The first 10 rows displayed  by the commandabove

**How many rows are in this data file?**

**Show the first lines of the data**
```{bash}
gzcat /Users/Aoki/mimic/core/admissions.csv.gz | head -3
```
**Show the last lines of the data**
```{bash}
gzcat /Users/Aoki/mimic/core/admissions.csv.gz | tail -3
```
Based on the result we know that the first line is the name of the variables and start from the second line, each line is a patient.  

```{bash}
gzcat /Users/Aoki/mimic/core/admissions.csv.gz | wc -l 
```
